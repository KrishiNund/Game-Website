...
function submitForm(){
    ...
    let genderSelected = document.querySelector("input.gender-options:checked").value;
    ...
    if (age < 12){
        warningText += "You must be at least <b>12 years old</b> to play this game!s <br>";
        Swal.fire({
    ...
    //if user meets age requirement, move on to this part
        ...
        if (username.length > 15){
            errorText += "Max username length is <b>13 characters!</b> <br>";
            errorCount++;
        }
        //checking if password contains at least 1 uppercase character
        if (!/[A-Z]/.test(password)){
            errorText += "Password must contain at least 1 <b>uppercase character!</b> <br>";
            errorCount++;
        }
        //checking if password contains at least 1 lowercase character
        if (!/[a-z]/.test(password)){
            errorText += "Password must contain at least 1 <b>lowercase character!</b> <br>";
            errorCount++;
        }
        ...
        //if no error, store user data
            ...
            let userAlreadyExists = usersArray.some(user => user.Username === username);
